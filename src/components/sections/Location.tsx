  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 md:mt-16 md:gap-10 lg:grid-cols-5">
          {/* Address card */}
          <motion.div
            variants={fadeUp}
            initial="hidden"
            whileInView="visible"
            viewport={VIEWPORT}
            className="flex flex-col gap-6 rounded-3xl border border-forest/10 bg-ivory-100 px-7 py-9 md:px-8 md:py-10 lg:col-span-2"
          >
          ...
          {/* Map — lazy loaded, no GPU overhead until visible */}
          <motion.div
            variants={fadeUp}
            initial="hidden"
            whileInView="visible"
            viewport={VIEWPORT}
            className="overflow-hidden rounded-3xl border border-forest/10 lg:col-span-3"
          >
            <iframe
              title={isAr ? "خريطة لاستر سبا" : "Lustre Spa map"}
              src={`https://www.google.com/maps?q=${encodeURIComponent(
                BRAND.mapsEmbedQuery
              )}&output=embed`}
              loading="lazy"
